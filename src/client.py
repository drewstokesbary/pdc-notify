# socrata_mailer/socrata/client.py
...
def fetch_page(limit: int = LIMIT, offset: int = 0, *, last_seen: Optional[str] = None) -> List[Dict[str, Any]]:
...
def fetch_page_resilient(
    ...
    *,
    ...
    params["$offset"] = str(int(offset))
    headers: Headers = socrata_headers()
    r = get_with_retries(BASE, params=params, headers=headers, timeout=timeout, max_attempts=max_attempts)
    return _normalize_rows(r.json())
